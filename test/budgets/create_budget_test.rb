...
class CreateBudgetTest < ActionController::TestCase
  def setup
    prepare_creating_object.create("valid", session.id)
  end
  ...
  def test_create_budget_should_raises_error_if_session_not_exist
    assert_raises(CreateBudget::SessionNotExist) { prepare_creating_object.create("valid", -2)}
  end

  def test_creat_budget_should_raises_error_if_name_is_too_short
    assert_raises(CreateBudget::InvalidBudgetName) {prepare_creating_object.create("aa", session.id)}
  end

  def test_creat_budget_should_raises_error_if_no_name_given
    assert_raises(CreateBudget::InvalidBudgetName) {prepare_creating_object.create(nil, session.id)}
  end
